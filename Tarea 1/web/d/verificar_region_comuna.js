...
$(document).ready(function () {
    var json = JSON.parse(regcom);
    var selRegion = $('#select-region');
    var selComuna = $('#select-comuna');
    var jsonRegiones = json.regiones;
    var regionesOpciones = "";

    $.each(jsonRegiones, function (k, v) {
        regionesOpciones += `<option value="${k}">${v.nombreRegion}</option>`;
    });
    ...
    $(selRegion).on('change', function () {
        var regionSeleccionada = selRegion[0][selRegion[0].selectedIndex].text;
        selRegion[0].setCustomValidity('Campo vacio o modificado');
        selComuna[0].setCustomValidity('Comuna no seleccionada o modificada');
        if (regionSeleccionada) {
            for (var key in jsonRegiones) {
                if (jsonRegiones[key].nombreRegion == regionSeleccionada) {
                    selRegion[0].setCustomValidity('');
                    selComuna.find("option:not(:first)").remove();
                    var comunaOpciones = "";
                    var jsonComunas = jsonRegiones[this.value].comunas;
                    $.each(jsonComunas, function (k, v) {
                        comunaOpciones += `<option value="${k}">${v}</option>`;
                    });
    ...
    $(selComuna).on('change', function () {
        ...
        if (regionSeleccionada) {
            var comunaSeleccionada = selComuna[0][selComuna[0].selectedIndex].text;
            for (var key in jsonRegiones) {
                if (jsonRegiones[key].nombreRegion === regionSeleccionada) {
                    for (var keyCom in jsonRegiones[key].comunas) {
